...
public class udpClient implements Runnable{
	...
	public udpClient(UDPClientListener listener) {
		super();
		this.addUDPClientListener(listener);
	}
	...
	public void run(){
		...
		while (!udpSocket.isClosed()) {
			...
			try {
				...
				this.onMessageReceivedUDPClient(c, id.replace("/", ""));
	...
	public static interface UDPClientListener extends EventListener{
		void onMessageReceivedUDPClient(byte b[], String id);
	}	
	...
	protected void onMessageReceivedUDPClient(byte b[], String id){
		for(UDPClientListener listener:listeners)
			listener.onMessageReceivedUDPClient(b, id);
	}	
}
