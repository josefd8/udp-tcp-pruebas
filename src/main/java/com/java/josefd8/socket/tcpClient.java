...
public class tcpClient implements Runnable{
	...
	public void run() {
		...
		while (!socket.isClosed()) {
			try {
				...
				this.onErrorOcurredTcpClient("tcpClient: " + e.getMessage());
				try {
					socket.close();
					break;
				} catch (IOException e1) {
					System.out.println("tcpClient: socket.close() " + e.getMessage());
				}
			}
		}	
	}
	...
	public static interface TCPClientListener extends EventListener{
		...
		void onErrorOcurredTcpClient(String message);
	}	
	...
	protected void onErrorOcurredTcpClient (String message){
		for(TCPClientListener listener:listeners)
			listener.onErrorOcurredTcpClient(message);
	}
}
